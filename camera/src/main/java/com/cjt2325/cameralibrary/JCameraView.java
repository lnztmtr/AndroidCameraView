...
//    private static final String TAG = "JCameraView";
    ...
    private int takePictureWidth = 1920;       //拍照图片尺寸，宽度
    private int takeVideoWidth = 1920;       //拍视频尺寸，宽度
    ...
    public JCameraView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        takePictureWidth = a.getInteger(R.styleable.JCameraView_takePictureWidth, 1920);       //没设置默认为1920
        takeVideoWidth = a.getInteger(R.styleable.JCameraView_takeVideoWidth, 1920);       //没设置默认为1920
        a.recycle();
        initData();
        initView();
    }
    ...
    private void initData() {
        ...
        LogUtil.i("layout_width = " + layout_width);
        //缩放梯度
    ...
    private void initView() {
        ...
        mPhoto = (PhotoView) view.findViewById(R.id.image_photo);
...
//                    Log.i("CJT", "result = " + (result - firstTouchLength));
    ...
    public boolean handlerFoucs(float x, float y) {
        //此处与captureLayout位置紧耦合了
        LayoutParams layoutParams = (LayoutParams) mCaptureLayout.getLayoutParams();
        if (layoutParams.gravity == Gravity.BOTTOM) {
            if (y > mCaptureLayout.getTop()) {
                return false;
            }
        } else if (layoutParams.gravity == Gravity.TOP) {
        ...
        if (layoutParams.gravity == Gravity.BOTTOM) {
            ...
            if (x > mCaptureLayout.getLeft() - mFoucsView.getWidth() / 2) {
                x = mCaptureLayout.getLeft() - mFoucsView.getWidth() / 2;
            }
            if (y < mFoucsView.getWidth() / 2) {
                y = mFoucsView.getWidth() / 2;
            }
